...
class DieselPumpLine(models.Model):
	...
	previous_odometer = fields.Float('previous fill Reading',compute="compute_previous_odometer_1",store=True,readonly=False)
	...
	def _onchange_vehicle_id(self):
		for rec in self:
			previuos_record = self.search([('vehicle_id', '=', rec.line_id2.vehicle_no.id)], limit=1, order="id desc")
			# prev =

			if previuos_record:
				pass
				# rec.previous_odometer = previuos_record.odometer

	@api.one
	@api.depends('vehicle_id')
	def compute_previous_odometer_1(self):
		for rec in self:
			previuos_record = self.search([('vehicle_id', '=', rec.line_id2.vehicle_no.id)], limit=2, order="id desc")
			prev = self.search([('id', '=', previuos_record.ids)], limit=1, order="id desc")
			if prev.id != rec.id:
				rec.previous_odometer = prev.closing_odometer
			else:
				prev = self.search([('id', '=', previuos_record.ids)], limit=1, order="id asc")
				rec.previous_odometer = prev.closing_odometer
...
class DriverDailyStatement(models.Model):
	...
	is_a_mach = fields.Boolean(string="Is a Machinery",related='vehicle_no.is_a_mach')
	night_shift = fields.Boolean(string="Second Driver of this Vehicle for this day?")
...
class DriverDailyStatementLine(models.Model):
	...
	# d_line_id = fields.Many2one('driver.daily.statement')
	...
	def _onchange_end_km(self):
		if self.end_km and self.start_km:
			if self.line_id.vehicle_no.is_a_mach:
				self.rent = self.line_id.vehicle_no.rent_amount * round(self.end_km - self.start_km, 2)
				# self.rent = self.line_id.vehicle_no.rate_per_km * round(self.end_km - self.start_km,2)
			else:
	...
	def compute_total(self):
		for rec in self:
			...
			if rec.line_id.vehicle_no.is_a_mach:
				rec.rent = rec.line_id.vehicle_no.rent_amount * round(rec.end_km - rec.start_km, 2)
				# rec.rent = self.line_id.vehicle_no.rate_per_km * round(rec.end_km-rec.start_km,2)
			else:
				rec.rent = rec.line_id.vehicle_no.rent_amount
	...
	def compute_previous_odometer(self):
		for rec in self:
			print("inside compute")
			# stmt_obj= self.env['driver.daily.statement'].search([('')])
			previuos_record = self.search([('vehicle_no', '=', rec.line_id.vehicle_no.id)], limit=2, order="id desc")
			prev = self.search([('id', '=', previuos_record.ids)], limit=1, order="id desc")
			if prev.id != rec.id:
				rec.previous_km = prev.end_km
			else:
				prev = self.search([('id', '=', previuos_record.ids)], limit=1, order="id asc")
				rec.previous_km = prev.end_km
	...
	previous_km = fields.Float(string='Previous KM/Hr', compute='compute_previous_odometer', store=True,readonly=False)
	start_time = fields.Float()
	end_time = fields.Float()
	start_date = fields.Date()
	end_date = fields.Date()
	time_1 = fields.Selection([('am', 'AM'), ('pm', 'PM')])
	time_2 = fields.Selection([('am', 'AM'), ('pm', 'PM')])
	...
	rent = fields.Float('Rent', compute='compute_total')
	remarks  = fields.Char('Remarks')
	start_km = fields.Float('Start KM')
	end_km = fields.Float('End KM')
	driver_betha = fields.Float('Driver Amount', compute = "compute_driver_bata")
	contractor_id = fields.Many2one('res.partner',domain="[('contractor', '=', True)]", string='Contractor')
	...
	@api.onchange('start_date', 'end_date', 'start_time', 'end_time', 'time_1', 'time_2')
	def onchange_time(self):
		for rec in self:
			if rec.start_date and rec.end_date and rec.start_time and rec.end_time and rec.time_1 and rec.time_2:
				list_start_time = str(rec.start_time).split(".")
				# if len(list_start_time[0])>2 or len(list_start_time[1])>2:
				# 	raise osv.except_osv(_('Error!'), _("please time"))
				if len(list_start_time) == 2:
					if len(list_start_time[1]) == 1:
						list_start_time[1] = list_start_time[1] + "0"
					end_string_start_time = ":" + list_start_time[1] + ":00"
				else:
					end_string_start_time = ":00:00"
				if rec.time_1 == 'am':
					first_string_start_time = list_start_time[0]
				if rec.time_1 == 'pm':
					first_string_start_time = str(int(list_start_time[0]) + 12)
				string_start_time = first_string_start_time + end_string_start_time
				start_time = datetime.datetime.strptime(str(rec.start_date)+ " "+ string_start_time, "%Y-%m-%d %H:%M:%S")

				list_end_time = str(rec.end_time).split(".")
				# if len(list_end_time[0])>2 or len(list_end_time[1])>2:
				# 	raise osv.except_osv(_('Error!'), _("please time"))
				if len(list_end_time) == 2:
					if len(list_end_time[1]) == 1:
						list_end_time[1] = list_end_time[1] + "0"
					end_string_end_time = ":" + list_end_time[1] + ":00"
				else:
					end_string_end_time = ":00:00"
				if rec.time_2 == 'am':
					first_string_end_time = list_end_time[0]
				if rec.time_2 == 'pm':
					first_string_end_time = str(int(list_end_time[0]) + 12)
				string_end_time = first_string_end_time + end_string_end_time
				end_time = datetime.datetime.strptime(str(rec.end_date) + " " + string_end_time,
														"%Y-%m-%d %H:%M:%S")
				# user_tz = self.env.user.tz or pytz.utc
				# local = pytz.timezone(user_tz)
				# start_time = datetime.datetime.strptime(datetime.datetime.strftime(pytz.utc.localize(
				# 	datetime.datetime.strptime(rec.start_time, "%Y-%m-%d %H:%M:%S")
				# ).astimezone(local), "%d/%m/%Y %H:%M%S"), "%d/%m/%Y %H:%M%S")
				# end_time = datetime.datetime.strptime(datetime.datetime.strftime(pytz.utc.localize(
				# 	datetime.datetime.strptime(rec.end_time, "%Y-%m-%d %H:%M:%S")
				# ).astimezone(local), "%d/%m/%Y %H:%M%S"), "%d/%m/%Y %H:%M%S")
				if start_time > end_time:
					raise osv.except_osv(_('Error!'), _("End time should be greater than Start time"))
				shift_end_time = datetime.datetime.strptime((str(rec.start_date) + " 19:00:00"), "%Y-%m-%d %H:%M:%S")
				shift_start_time = datetime.datetime.strptime((str(rec.start_date) + " 07:00:00"), "%Y-%m-%d %H:%M:%S")
				if end_time <= shift_end_time:
					rec.driver_betha = rec.driver_name.per_day_eicher_rate
					rec.over_time=0
				else:
					difference = end_time - shift_end_time
					over_time = 0
					if difference.days > 0:
						over_time = difference.days * 24.0
						list_difference = str(difference).split(",")
						over = str(list_difference[1]).split(':')
						# if float(over[1]) >30:
						hours = float((float(over[0]) * 60) + float(over[1])) + over_time*60
					else:
						over_time = str(difference).split(":")
						hours = (float(over_time[0]) * 60) + float(over_time[1])
					# early_overtime = str(start_time - shift_start_time).split(":")
					# early_hours = (float(early_overtime[0]) * 60) + float(early_overtime[1])
					driver_betha = rec.driver_name.per_day_eicher_rate
					if hours > 0:
						rec.over_time = hours/60
						over_time_amount = hours/60 * 50
						driver_betha += over_time_amount
					# if early_overtime > 0:
					# 	early_over_time_amount = float(early_hours[0] + '.' + early_hours[1]) * 50
					# 	driver_betha += early_over_time_amount
					rec.driver_betha = round(driver_betha,2)

	@api.depends('start_time', 'end_time', 'time_1', 'time_2')
	def compute_driver_bata(self):
		for rec in self:
			if rec.start_date and rec.end_date and rec.start_time and rec.end_time and rec.time_1 and rec.time_2:
				list_start_time = str(rec.start_time).split(".")
				# if len(list_start_time[0])>2 or len(list_start_time[1])>2:
	...
	@api.model
	def create(self,vals):
		# if 'start_time' and 'end_time' in vals:
			# user_tz = self.env.user.tz or pytz.utc
			# local = pytz.timezone(user_tz)
			# start_time = datetime.datetime.strptime(datetime.datetime.strftime(pytz.utc.localize(
			# 	datetime.datetime.strptime(vals['start_time'], "%Y-%m-%d %H:%M:%S")
			# ).astimezone(local), "%d/%m/%Y %H:%M%S"), "%d/%m/%Y %H:%M%S")
			# end_time = datetime.datetime.strptime(datetime.datetime.strftime(pytz.utc.localize(
			# 	datetime.datetime.strptime(vals['end_time'], "%Y-%m-%d %H:%M:%S")
			# ).astimezone(local), "%d/%m/%Y %H:%M%S"), "%d/%m/%Y %H:%M%S")
			# if start_time > end_time:
			# 	raise osv.except_osv(_('Error!'), _("End time should be greater than Start time"))
